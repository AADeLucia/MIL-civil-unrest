...
OUTPUT_DIR="${BASE_OUTPUT_DIR}/beta_${b}"
LOG_DIR="${OUTPUT_DIR}/logs"
mkdir -p "${LOG_DIR}"
export WANDB_PROJECT="Minerva"
export WANDB_RUN_GROUP="MIL-BI-Min10"
...
WARMUP=100
# Start training
echo "Training model with instance loss beta  ${b}. Saving to ${OUTPUT_DIR}"
torchrun --nproc_per_node 4 "${MINERVA_HOME}/code/train_mil.py" \
  ...
  --run_name "instance-loss-${b}" \
  --instance_level_loss "${b}" \
